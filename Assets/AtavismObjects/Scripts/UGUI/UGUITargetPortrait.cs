...
namespace Atavism
{
    ...
    public class UGUITargetPortrait : MonoBehaviour, IPointerClickHandler
    {
        ...
        [Header("P4 Addons")]
        private string prop;
        public AtavismGuild atavismGuild;
        AtavismRegion regionPvP;
        AtavismMecanimMobController3D AtavismMecanim;
        Atavism3rdPersonInput inputPerson;
        ...
        AtavismSettings atavismSettings;
        ...
        void Start()
        {
            ...
            // Obtenha uma referência para a instância da classe AtavismGuild
            atavismGuild = AtavismGuild.Instance;
        ...
        void CheckTargetHealth()
        {
            ...
            if (ClientAPI.GetTargetObject() != null) ;
            {
                int health = (int)ClientAPI.GetTargetObject().GetProperty("health");
        ...
        //FUNCIONA PARA VERIFICAR SE OS PLAYERS SÃO DA MESMA GUILD
        private void CheckGuildPvpState()
        {
            ...
            if (playerOidValue != 0 && targetOidValue != 0 && targetOidValue != playerOidValue)
            {
                ...
                if (guild == null)
                {
                    Debug.Log("Guild instance is null");
                    return;
                }
                ...
                {
                    //Debug.Log("Nenhum dos dois são da mesma guild");
                    return;
                }
                ...
                if (playerGuildName != null && playerGuildName == targetGuildName && currentScene != "Arena1v1")
                {
                    //Debug.Log("DESLIGOU O PVP :DDD" + " eles são da msm guild " + targetGuildName);
                    NetworkAPI.SendAttackMessage(ClientAPI.GetPlayerOid(), "strike", false);

                    // Configura as propriedades para desativar o estado PvP
                    Dictionary<string, object> props = new Dictionary<string, object>();
                    props.Add("pvpState", false);

                    // Envia uma mensagem de extensão para atualizar o estado PvP do jogador
        ...
        private void CheckGroupPvpState()
        {
            long playerOidValue = ClientAPI.GetPlayerOid();
            long targetOidValue = ClientAPI.GetTargetOid();
            ...
            if (playerOidValue != 0 && targetOidValue != 0 && targetOidValue != playerOidValue)
            {
                ...
                AtavismGroup group = AtavismGroup.Instance;
                if (group == null)
                {
                    Debug.Log("Group instance is null");
                    return;
                }

                GroupMember playerMember = group.Members.Find(member => member.oid == playerOid);
                GroupMember targetMember = group.Members.Find(member => member.oid == targetOid);

                if (playerMember == null || targetMember == null)
                {
                    // Os jogadores não estão no mesmo grupo
                    return;
                }

                // Desativa o PvP se os jogadores estão no mesmo grupo
                if (playerMember.properties.ContainsKey("groupID") && targetMember.properties.ContainsKey("groupID"))
                {
                    int playerGroupID = (int)playerMember.properties["groupID"];
                    int targetGroupID = (int)targetMember.properties["groupID"];

                    if (playerGroupID == targetGroupID)
                    {
                        Debug.Log("Desligou o PvP, eles estão no mesmo grupo");

                        // Envia uma mensagem para desativar o PvP
                        Dictionary<string, object> props = new Dictionary<string, object>();
                        props.Add("pvpState", false);
                        NetworkAPI.SendExtensionMessage(ClientAPI.GetPlayerOid(), false, "faction.UPDATE_PVP_STATE", props);
                    }
        ...
        public void DistanceToMobSKILL()
        {
            if (Vector3.Distance(ClientAPI.GetPlayerObject().Position, ClientAPI.GetObjectNode(ClientAPI.GetTargetOid()).Position) > AtavismTrade.Instance.InteractionDistance)
            {
                
                // Obter as posições do jogador e do alvo
                Vector3 playerPos = ClientAPI.GetPlayerObject().Position;
                Vector3 targetPos = ClientAPI.GetObjectNode(ClientAPI.GetTargetOid()).Position;

                // Calcular a distância
                float distance = Vector3.Distance(playerPos, targetPos);

                // Verificar se a distância é maior que InteractionDistance
                if (distance > AtavismTrade.Instance.InteractionDistance)
                {
                    // Calcular a diferença
                    float difference = distance - AtavismTrade.Instance.InteractionDistance;
                    Debug.Log(AtavismTrade.Instance.InteractionDistance);
                    // Imprimir a diferença
                    Debug.Log("A distância entre o jogador e o alvo é " + difference + " unidades maior que a InteractionDistance.");
        ...
        public void DistanceToMob()
        {
            if (Vector3.Distance(ClientAPI.GetPlayerObject().Position, ClientAPI.GetObjectNode(ClientAPI.GetTargetOid()).Position) > AtavismTrade.Instance.InteractionDistance)
            {
                ...
                if (distance < AtavismTrade.Instance.InteractionDistance)
                {
                    Debug.Log("O jogador está dentro da InteractionDistance.");
                    Debug.Log(AtavismTrade.Instance.InteractionDistance);
                }
            }
        ...
        public void FollowMob()
        {
            NetworkAPI.SendTargetedCommand(ClientAPI.GetPlayerOid(), "/gotopos " + ClientAPI.GetTargetObject().Position);
            Debug.Log("Consegui Caraioo");
        }
