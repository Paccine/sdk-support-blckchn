...
//                UnityEngine.Debug.LogWarning("MobNode.ComputePosition: oid " + oid + ", followTerrain " + followTerrain + ", pathInterpolator " + (locAndDir == null ? "null" : locAndDir.ToString ())+" Pos="+ transform.position+" interpolateSpeed="+interpolateSpeed);
       ...
       /*     movement = movement.normalized * speed;
        ...
        /// <summary>
        /// Verifica a distância entre o jogador e o alvo. Move o jogador até o alvo somente se a distância for maior que a InteractionDistance.
        /// </summary>
        /// <summary>
        /// Verifica a distância entre o jogador e o alvo. Move o jogador até o alvo somente se a distância for maior que a InteractionDistance.
        /// </summary>
        ...
        {
            // Obter a posição do jogador e do alvo
            ...
            if (distance > AtavismTrade.Instance.InteractionDistance)
            {
                ...
                // Mover o jogador em direção ao alvo
                MovePlayerTowardsTarget(playerPos, targetPos);
            }
            else
            {
                // O jogador já está dentro da InteractionDistance
        ...
        /// </summary>
        /// <param name="playerPos">A posição do jogador.</param>
        /// <param name="targetPos">A posição do alvo.</param>
        private void MovePlayerTowardsTarget(Vector3 playerPos, Vector3 targetPos)
        {
            // Calcular a direção para o alvo
            Vector3 directionToTarget = targetPos - playerPos;
            directionToTarget.y = 0f; // Ignorar a componente y para evitar inclinação indesejada

            // Normalizar a direção para ter magnitude igual a 1
            directionToTarget.Normalize();

            // Definir a velocidade de movimento do jogador (você pode usar sua própria velocidade desejada aqui)
            float moveSpeed = 5f;

            // Mover o jogador em direção ao alvo
            playerPos += directionToTarget * moveSpeed * Time.deltaTime;
            ClientAPI.GetPlayerObject().Position = playerPos;
        }
    }
}